...
## Keeping files small
- If there any many tests for interactor / api route - divide them into multiple modules. Don't keep them all in one file.
- Divide based on route / micro-domain / business-action.
- Name API endpoint test files like `test_get_notes.py` for `GET /api/notes/` endpoint. Don't add `_views` or `_api` in the end of file name.
